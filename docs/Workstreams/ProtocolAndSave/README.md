...
8. NPC 两段式日计划通过 `NPC_DAY_PLAN_READY` 携带 operation、day、plan revision、planner version、segments 与两个有序队列；运行时 effect/result 继续携带稳定关联字段。旧日程 replacement/replan、`NPC_BEHAVIOR`、`NPC_TASK_STATUS*`、`NPC_ACTION_RESULT*`、旧时间控制和 wake/sleep push 无生产入口。
9. Unity 主存档当前 schema 为 3；DTO 默认版本、仓储写入校验目标和迁移链终点必须一致。两段式队列独立测试发现的 schema 2/3 不一致已完成实现修复，真实跨日提交仍待独立复测。
...
4. Unity 主存档框架：已完成。
5. 双端保存 / 读档事务：已完成。
6. 重连与世界快照：世界快照协议已完成；运行中后端重启会被 Unity 立即升级为致命退出，自动重连状态机缺失，详见 `BackendReconnectFatalDraft.md`。
...
4. 双进程保存、覆盖保存、加载和失败恢复既有回归已完成；2026-07-19 复测确认运行中后端重启无法自动重连，旧“断线重连完成”结论撤回。

当前状态：状态所有权、envelope 和 checkpoint 契约保持稳定；连接恢复状态机重新列为待设计事项，其他底座边界不重新打开。

开发期临时策略：每次确认进入新游戏时，Unity 通过 `new_game_backend_purge` 统一清除 Python 记忆检查点、可重建日程快照和同日内存幂等缓存，收到完整成功响应后再清除 Unity 全部主存档；任一子域失败均不得进入新游戏。旧 `memory_checkpoints_purge_all` 仅保留兼容入口。该策略不改变 Unity 世界权威、Python 记忆权威的边界。
...
1. `BackendReconnectFatalDraft.md`：记录运行中后端重启被立即升级为致命退出、未进入新 session 握手的问题与后续设计边界。
