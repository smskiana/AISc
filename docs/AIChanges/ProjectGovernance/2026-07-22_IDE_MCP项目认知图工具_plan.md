...
阶段 C 的主展示形态是分层 UML 认知图，而不是“功能域树 + 关系表”。该视图借用 UML 的分类器节点和关系箭头表达项目结构，但不复制传统类图的属性区、方法区：

1. 概览层展示当前功能域的直属子功能域和直属类；不把全部后代或全项目节点铺在同一张图中。
2. 子功能域使用可下钻的功能域节点；点击后以该功能域为新的当前层级重新绘图。
3. 顶部使用面包屑表达当前层级，并允许返回任一父功能域。
4. 类节点只显示类名、类型和简要职责，不显示属性清单或方法清单。
5. 中央视图显示当前层级节点之间的主要关系；选中类时可以切换为该类的一跳中心图。
6. 继承、接口实现、调用或依赖使用可区分的 UML 箭头和线型；关系语义不能只依赖颜色。
7. 点击关系展开具体事实证据；方法仅作为关系证据出现，并提供文件与行号源码跳转，不在类节点中重复罗列。
8. 双击类节点或使用明确的源码命令直接打开类定义。
9. 功能域树和 `待归类` 只作为范围选择、搜索和治理入口，不再作为主展示结果。
10. 图布局使用成熟的图布局能力，不手写节点排布算法；VS Code adapter 通过 Webview 承载交互图，具体图引擎在执行会话根据现有依赖和 bundle 约束冻结。

其余 IDE 操作要求：

1. 显示 AI 建议、人工确认、过期和冲突状态。
2. 从当前符号、当前文件、当前功能域发起局部认知更新。
3. 在应用前显示范围、排除域、跨域只读证据和认知差异。
4. 全面更新作为独立、低频命令，不放在普通刷新按钮的直接路径上。

UML 认知图默认限制可见节点和边，优先展示 importance 较高的主要关系。层级下钻、类中心一跳视图和关系证据展开是三种不同操作：下钻改变当前功能域层级，类中心视图改变局部关系焦点，证据展开只显示关系事实而不继续扩张图。任何操作都不能隐式渲染整个项目。
...
1. 实现分层 UML 认知图、功能域下钻、父级面包屑返回、类中心一跳视图、关系证据展开和源码跳转。
2. 类节点以“类名 + 类型 + 简要职责”替代属性区和方法区；方法级内容仅进入关系证据。
3. 实现范围预览与差异确认。
4. 验证分层图按需下钻、局部关系展开和渲染预算。
5. 保持 MCP Server 可由 AI 客户端独立调用。
...
9. 若阶段 C 实现 IDE adapter，增加 IDE 扩展宿主集成测试和桌面截图验证，检查 UML 节点与箭头、功能域下钻、父级返回、类中心视图、关系展开、长名称、窄窗口和预算上限节点数量下无重叠。
...
1. IDE 主视图以分层 UML 认知图展示当前功能域的直属子功能域、直属类及其主要关系，而不是关系表或一次性全项目网络。
2. 点击子功能域可以进入下一层；面包屑可以返回任一父功能域，往返后当前层级和选择状态一致。
3. 类节点只展示类名、类型和简要职责，不出现属性区或方法区；长职责文本有明确截断和查看完整内容的方式。
4. 选中类可以查看该类的一跳关系；继承、接口实现、调用或依赖具有可辨识且符合 UML 习惯的箭头或线型。
5. 点击关系可以展开方法级证据并跳转到源代码；方法不会在类节点中重复展示。
6. 发起更新时能清楚看到目标域、包含符号、排除域、跨域只读证据和预计成本。
7. 默认视图不会绘制全项目图；节点和边受预算约束，窄窗口、长名称和预算上限节点数量下不得发生不可读重叠。
8. IDE adapter 移除后，MCP Server 仍能被测试客户端和 AI 客户端独立使用。
...
10. **分层导航与代码关系混淆**：功能域父子关系只控制层级下钻，代码继承、实现和调用只作为 UML 关系边；两者使用不同节点类型和交互，不把目录层级伪装成 UML 继承。
