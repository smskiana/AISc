...
- `task_catalog.py`: 共享 action-location-role affordance 查询与校验，并统一投影两段式任务的 segment、完成策略、抢占策略、Gameplay 持续量和生命周期动作标记。
- `daily_schedule.py`: 冻结时间/世界版本下生成 work/rest 两个有序候选队列；普通任务不含精确时间点，互动后完整 replacement 已退役。
- `schedule_candidates.py`、`schedule_memory_evidence.py`、`schedule_validation.py`: 分别承载物理合法候选、按组图/向量证据和最终硬校验；不把候选细节回塞给 `behavior_engine.py`。
...
日程 planner 的 JSON 输出只包含 `work_tasks[] / rest_tasks[]` candidate ID；adapter 不接受时间、duration、完成条件、移动或失败策略，未知、跨段、重复和缺失 required group 整体 fallback。required 是显式必选组，不要求同组所有地点候选全部出现；Unity 的活动任务、边界、mutation、P0/need 和运行时状态不持久化到 Python。
候选证据只保存记忆 ID、向量相似度、图路径分与 retrieval trace ID；不保存 embedding。可通过 `/api/npc/daily_schedule_trace` 读取安全 owner trace。
计划与社交意愿 Prompt 使用 `src/prompting.PromptAssembler`；`_task_catalog.validate_task` 与本地 fallback 仍是最终约束。
