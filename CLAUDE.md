...
**每个新对话开始时，AI 必须首先读取以下文件了解当前项目状态：**

1. **`docs/ProjectIndex.md`** — 项目总入口（推荐阅读顺序、核心目录、规范入口）
2. **`docs/AIChanges/` 下按日期最新的 `HANDOFF_YYYY-MM-DD.md`** — 最新会话的交接记录（当前状态、待处理任务、已知问题、下次建议起点）
   - 当前应优先读取：`docs/AIChanges/Archive/Handoffs/HANDOFF_2026-07-11.md`
3. **`docs/AIChanges/` 下按日期最新、且与当前任务最相关的 `_execution.md`** — 最近一次相关修改的详细清单
4. **`docs/AIChanges/` 目录** — 检查是否有未在交接文档中反映的新变更

交接文档和最新执行记录构成“会话感知链”——每个新对话通过这些文件了解之前做了什么、卡在哪里、接下来该做什么。若发现 handoff 已落后于目录中的新变更，应先补交接链，再继续开发。
...
**每次非平凡修改必须产生两份文档，放在 `docs/AIChanges/` 下：**

| 文件 | 命名 | 内容 |
|------|------|------|
| 设计方案 | `YYYY-MM-DD_<主题>_plan.md` | 需求理解、方案思路、涉及文件、风险点 |
| 执行记录 | `YYYY-MM-DD_<主题>_execution.md` | 实际改动清单、遇到的问题、验证方式、未完成项 |

**互相索引规则：**
- `_plan.md` 顶部写：`> 执行记录: [execution.md](YYYY-MM-DD_<主题>_execution.md)`
- `_execution.md` 顶部写：`> 设计方案: [plan.md](YYYY-MM-DD_<主题>_plan.md)`

**触发时机：**
- 新建文件 ≥2 个，或修改文件 ≥3 个 → 必须先写 plan，改完写 execution
- 单文件小修（改几行修 bug）→ 可跳过 plan，但事后在 execution 记一笔
- 纯探索/读代码 → 不需要

**AI 在开始任何修改前，必须先检查 `docs/AIChanges/` 是否有相关历史文档可参考。**
