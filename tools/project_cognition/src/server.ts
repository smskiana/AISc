...
import { CodeSymbol, MembershipType, ProjectSnapshot } from "./domain/model.js";
...
import { buildDomainOverview } from "./query/domain-overview.js";
import { resolveScope, ScopeBudget, ScopeResolution } from "./scope/scope-engine.js";
import { buildScopeView, ScopeView } from "./scope/scope-view.js";
...
interface SignedScope { resolution: ScopeResolution; symbols: CodeSymbol[]; view: ScopeView; }
const scopes = new Map<string, SignedScope>();
...
server.registerTool("find_symbol", { description: "Find language-independent symbols through the configured code graph.", inputSchema: { projectId: z.string(), query: z.string(), filePath: z.string().optional(), symbolName: z.string().optional(), kind: z.enum(["Class", "Interface", "Struct", "Enum", "Record"]).optional(), limit: z.number().int().min(1).max(200).default(50) } }, args => respond(() => graph.resolveSymbols(args.projectId, args.query, args.limit, args.filePath, args.symbolName, args.kind)));
server.registerTool("get_classifier_context", { description: "Read bounded class-to-class CALLS relations aggregated from method evidence.", inputSchema: { projectId: z.string(), qualifiedName: z.string(), limit: z.number().int().min(1).max(80).default(40) } }, args => respond(() => graph.getClassifierContext(args.projectId, args.qualifiedName, args.limit)));
server.registerTool("get_domain_overview", { description: "Read the current project cognition snapshot and domain tree.", inputSchema: { projectId: z.string(), domainId: z.string().optional(), depth: z.number().int().min(0).max(10).default(3) } }, args => respond(async () => buildDomainOverview(await store.load(args.projectId), args.domainId)));
...
server.registerTool("resolve_update_scope", { description: "Resolve and sign a budget-limited local cognition update scope.", inputSchema: { projectId: z.string(), query: z.string(), filePath: z.string().optional(), symbolName: z.string().optional(), domainId: z.string().optional(), maxFiles: z.number().int().min(1).max(100).default(20), maxSymbols: z.number().int().min(1).max(500).default(30), maxRelations: z.number().int().min(1).max(5000).default(100) } }, args => respond(async () => {
  const snapshot = await store.load(args.projectId);
  const budget: ScopeBudget = { maxFiles: args.maxFiles, maxSymbols: args.maxSymbols, maxRelations: args.maxRelations };
  const found = await graph.resolveSymbols(args.projectId, args.query, args.maxSymbols, args.filePath, args.symbolName);
  const resolution = resolveScope({ projectId: args.projectId, query: args.query, seedSymbols: found.symbols, requestedDomainId: args.domainId, codeRevision: await revisions.getRevision() }, snapshot, budget);
  const view = buildScopeView(snapshot, resolution, found.symbols, budget, args.domainId);
  scopes.set(resolution.scopeId, { resolution, symbols: found.symbols, view });
  return { ...resolution, ...view };
}));
server.registerTool("preview_scoped_update", { description: "Preview signed changes without writing the snapshot.", inputSchema: { projectId: z.string(), scopeId: z.string(), query: z.string().optional() } }, args => respond(async () => {
  const signed = scopes.get(args.scopeId);
  if (!signed) throw new Error("INVALID_SCOPE_ID");
  const snapshot = await store.load(args.projectId);
  const preview = previewUpdate(signed.resolution, snapshot, signed.symbols, await revisions.getRevision());
  previews.set(preview.id, preview);
  return { ...preview, ...signed.resolution, ...signed.view };
}));
server.registerTool("apply_scoped_update", { description: "Atomically apply accepted changes inside a signed scope.", inputSchema: { projectId: z.string(), scopeId: z.string(), previewId: z.string(), acceptedChangeIds: z.array(z.string()) } }, args => respond(async () => { const signed = scopes.get(args.scopeId); const preview = previews.get(args.previewId); if (!signed) throw new Error("INVALID_SCOPE_ID"); if (!preview) throw new Error("INVALID_PREVIEW_ID"); return applyUpdate(store, await store.load(args.projectId), signed.resolution, preview, args.acceptedChangeIds, await revisions.getRevision()); }));
server.registerTool("request_full_update", { description: "Issue a separately confirmed full-project scope.", inputSchema: { projectId: z.string(), explicitConfirmation: z.string(), query: z.string().default("class function method"), maxFiles: z.number().int().min(1).max(2000), maxSymbols: z.number().int().min(1).max(20000) } }, args => respond(async () => { const snapshot = await store.load(args.projectId); const found = await graph.resolveSymbols(args.projectId, args.query, args.maxSymbols); const budget: ScopeBudget = { maxFiles: args.maxFiles, maxSymbols: args.maxSymbols, maxRelations: args.maxSymbols * 5 }; const resolution = resolveScope({ projectId: args.projectId, query: args.query, seedSymbols: found.symbols, fullUpdate: true, explicitConfirmation: args.explicitConfirmation, codeRevision: await revisions.getRevision() }, snapshot, budget); const view = buildScopeView(snapshot, resolution, found.symbols, budget); scopes.set(resolution.scopeId, { resolution, symbols: found.symbols, view }); return { ...resolution, ...view }; }));
...
function registerResources(): void {
  server.registerResource("project-domains", new ResourceTemplate("project-cognition://projects/{projectId}/domains", { list: undefined }), { mimeType: "application/json" }, async (uri, vars) => { const overview = buildDomainOverview(await store.load(String(vars.projectId))); return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(overview.domains) }] }; });
  server.registerResource("project-domain", new ResourceTemplate("project-cognition://projects/{projectId}/domains/{domainId}", { list: undefined }), { mimeType: "application/json" }, async (uri, vars) => { const overview = buildDomainOverview(await store.load(String(vars.projectId)), String(vars.domainId)); return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify({ domain: overview.domains[0], memberships: overview.memberships, symbols: overview.symbols }) }] }; });
